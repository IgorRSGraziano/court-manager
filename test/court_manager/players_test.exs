defmodule CourtManager.PlayersTest do
  use CourtManager.DataCase

  alias CourtManager.Players

  describe "players" do
    alias CourtManager.Players.Player

    import CourtManager.PlayersFixtures

    @invalid_attrs %{is_gympass: nil, has_played: nil}
    ...
    test "create_player/1 with valid data creates a player" do
      valid_attrs = %{is_gympass: true, has_played: true}

      assert {:ok, %Player{} = player} = Players.create_player(valid_attrs)
      assert player.is_gympass == true
      assert player.has_played == true
    end
    ...
    test "update_player/2 with valid data updates the player" do
      player = player_fixture()
      update_attrs = %{is_gympass: false, has_played: false}

      assert {:ok, %Player{} = player} = Players.update_player(player, update_attrs)
      assert player.is_gympass == false
      assert player.has_played == false
    end
